...
\begin{abstract}
Certifying quantum nonlocality on real hardware requires knowing precisely how much noise a Bell state
can absorb before it loses its power to violate a Bell inequality — a question that has taken on
renewed urgency as NISQ devices move from proof-of-principle toward practical deployment.
Under global depolarizing noise of rate $p$, the CHSH parameter satisfies
$\bigS(p) = (1-p)\cdot 2\sqrt{2}$, and the boundary of violation is crossed at the exact threshold
$\pstar = 1 - 1/\!\sqrt{2} \approx 0.2929$ — a result that is non-perturbative and follows directly
from the tracelessness of Pauli matrices.
...
error below $0.001$.
Comparing three physically motivated noise channels, we find a clear hierarchy: local dephasing, which
selectively destroys the off-diagonal coherences responsible for CHSH violation, reaches its threshold
at $\pstar \approx 0.178$, while amplitude damping — which breaks the exchange symmetry of the Bell
state by driving both qubits toward $\ket{0}$ — crosses the boundary at $\pstar \approx 0.231$,
both strictly below the depolarizing threshold.
...
Translated to current superconducting hardware with two-qubit gate error rates near $0.5\%$, the
depolarizing threshold corresponds to a critical circuit depth of roughly 60 two-qubit gates, a figure
that directly constrains the entanglement depth available to variational algorithms and sets a firm
noise budget for device-independent quantum key distribution.
\end{abstract}
...
has been spectacularly confirmed by a succession of loophole-free experiments — from Aspect's
original photon pairs~\cite{aspect1982} to the 2015 Delft experiment closing both the locality
...
\end{equation}
while quantum mechanics allows $\bigS \leq 2\sqrt{2}$ — Tsirelson's bound~\cite{tsirelson1980} —
...
providing a clean cross-check of the analytical framework.
...
$1/f$ flux noise or charge noise in superconducting circuits — a dominant source of idle-time
...
in many superconducting platforms and introduces a directional asymmetry absent from both
...
\textit{Werner states}~\cite{werner1989} — a one-parameter family introduced precisely to
...
a result whose original derivation required more sophisticated techniques — a useful cross-check
on the analytical framework developed here.
...
violation — so dephasing is a targeted attack on entanglement in a way that depolarizing noise is not.
...
measurement angles — an effect that is absent from both depolarizing and dephasing noise.
...
Because the four Bell states are related to one another by exactly such local unitaries —
...
equation~\eqref{eq:optimal_angles}, and the surrounding landscape is a smooth plateau —
not a sharp peak.
...
for NISQ devices illustrates how the same mathematical structure — the Pauli algebra and its
tracelessness — carries from the foundations of quantum mechanics to practical questions in
quantum engineering.
...
That the answer is exact — not an approximation, not a numerically fitted curve — follows from
a remarkably simple algebraic fact: the tracelessness of Pauli matrices causes the maximally
...
Local dephasing, by targeting precisely the off-diagonal density matrix elements that carry
CHSH correlations, destroys Bell violation at $\pstar \approx 0.178$ — $40\%$ below the
...
for multipartite entangled states — GHZ states, graph states, or resource states for
...
Finally, connecting these thresholds to concrete error-mitigation overhead — how many
